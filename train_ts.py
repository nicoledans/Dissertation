...
def train_ts():
    ...
    # Exit if no TS masks in cache
    ts_count = sum(1 for n in nodules if n.get("ts_mask") is not None)
    if ts_count == 0:
        print(
            "No TotalSegmentator masks found in cache. Install then re-run."
        )
        return
    ...
    log_path = os.path.join(RESULTS_DIR, "train_ts_log.txt")
    best_auc = 0.0
    best_model_path = os.path.join(RESULTS_DIR, "best_model_ts.pt")
    ...
    if not val_nods:
        ...
        for images, masks, labels, _ in DataLoader(
            ...
            ax[3].set_title("Overlay + TS mask boundary")
            ax[3].axis("off")
        ...
        plt.tight_layout()
        fig.savefig(os.path.join(RESULTS_DIR, "gradcam_examples_ts.png"), dpi=150)
        plt.close(fig)
        print(f"Saved TS Grad-CAM examples to {RESULTS_DIR}/gradcam_examples_ts.png")
